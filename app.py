...
# ------------------- LOAD LOGO -------------------
logo_path = r"D:\Python_Projects\Protein_Lab\hrithik protein lab logo.png"
...
if page == "Dashboard":
    ...
    if not df.empty:
        ...
        if st.session_state.edit_id:
            ...
            if customer:
                col1, col2 = st.columns(2)
                name = col1.text_input("Name", customer[1])
                phone = col2.text_input("Phone", customer[2])

                col3, col4 = st.columns(2)
                start_date = col3.date_input("Start Date", pd.to_datetime(customer[3]))
                plan = col4.selectbox(
                    "Plan",
                    list(plans.keys()),
                ...
                # UPDATE
                if colA.button("✅ Update Customer"):
                    price, profit_percent, days = plans[plan]
                    profit = price * profit_percent
                    end_date = start_date + timedelta(days=days)
